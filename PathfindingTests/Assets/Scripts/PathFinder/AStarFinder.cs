...
namespace EpPathFinding.cs
{
    public class AStarParam : ParamBase
    {
        ...
        public AStarParam(BaseGrid iGrid, GridPos iStartPos, GridPos iEndPos, float iweight,
        ...
        public AStarParam(BaseGrid iGrid, float iweight, DiagonalMovement iDiagonalMovement = DiagonalMovement.Always,
        ...
        public static List<GridPos> FindPath(AStarParam iParam)
        {
            ...
            while (openList.Count != 0)
            {
...
#endif
                {
                    ...
                    var x = neighbor.x;
                    var y = neighbor.y;
                    float ng = node.startToCurNodeLen +
                               (float) ((x - node.x == 0 || y - node.y == 0) ? 1 : Math.Sqrt(2));
                    ...
                    if (!neighbor.isOpened || ng < neighbor.startToCurNodeLen)
                    {
                        ...
                        if (neighbor.heuristicCurNodeToEndLen == null)
                            neighbor.heuristicCurNodeToEndLen =
                                weight * heuristic(Math.Abs(x - endNode.x), Math.Abs(y - endNode.y));
