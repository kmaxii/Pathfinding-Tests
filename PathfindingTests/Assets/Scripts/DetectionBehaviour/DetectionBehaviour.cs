...
namespace DetectionBehaviour
{
    public abstract class DetectionBehaviour : ScriptableObject
    {
        ...
        protected IEnumerable<Vector2Int> GetNeighbours(Vector2Int position)
        {
            ...
            if (x < mapData.map.GetLength(0) - 1 && y < mapData.map.GetLength(1) - 1 && mapData.CheckCoordinate(x + 1, y + 1))
                neighbours.Add(new Vector2Int(x + 1, y + 1));
            if (x > 0 && y < mapData.map.GetLength(1) - 1 && mapData.CheckCoordinate(x - 1, y + 1))
                neighbours.Add(new Vector2Int(x - 1, y + 1));
            if (x < mapData.map.GetLength(0) - 1 && y > 0 && mapData.CheckCoordinate(x + 1, y - 1))
                neighbours.Add(new Vector2Int(x + 1, y - 1));
